...
## Devounce Time
We're going to modify our code to refresh input's value at the moment user modify the value. To do this we're going to use a rxj's observable called Subject wit hthe debounce's value. This process will start with the html input's value. This input element will call to a method called 'pressedKey()'.

```
<form (ngSubmit)="searchInput()" autocomplete="off">
    <input type="text"
        name="seacrh"
        class="form-control"
        [(ngModel)]="query"
...
```
We are going to have declared our debounc and rxjs observable in the class component.
```
...
```
NOTICE that we have used OnInit method that is called after Angular has initialized all data-bounded properties.